...
class ShoppingListForm(FlaskForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.name.label.text = trans('shopping_list_name', lang=lang) or 'List Name'
        self.budget.label.text = trans('shopping_budget', lang=lang) or 'Budget'
        self.submit.label.text = trans('shopping_submit', lang=lang) or 'Create List'
...
class ShoppingItemForm(FlaskForm):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        lang = request.args.get('lang', 'en')
        self.name.label.text = trans('shopping_item_name', lang=lang) or 'Item Name'
        self.quantity.label.text = trans('shopping_quantity', lang=lang) or 'Quantity'
        self.price.label.text = trans('shopping_price', lang=lang) or 'Price'
        self.unit.label.text = trans('shopping_unit', lang=lang) or 'Unit'
        self.store.label.text = trans('shopping_store', lang=lang) or 'Store'
        self.category.label.text = trans('shopping_category', lang=lang) or 'Category'
        self.status.label.text = trans('shopping_status', lang=lang) or 'Status'
        self.frequency.label.text = trans('shopping_frequency', lang=lang) or 'Frequency (days)'
        self.submit.label.text = trans('shopping_item_submit', lang=lang) or 'Add Item'
...
class ShareListForm(FlaskForm):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        lang = request.args.get('lang', 'en')
        self.email.label.text = trans('shopping_collaborator_email', lang=lang) or 'Collaborator Email'
        self.submit.label.text = trans('shopping_share_submit', lang=lang) or 'Share List'
...
def main():
    ...
    if request.method == 'POST':
        ...
        if action == 'create_list' and list_form.validate_on_submit():
            if not is_admin():
                if not check_ficore_credit_balance(db, required_amount=0.1, user_id=str(current_user.id), email=current_user.email):
            ...
            try:
                with db.client.start_session() as mongo_session:
                    with mongo_session.start_transaction():
                        list_id = create_shopping_list(db, list_data, session=mongo_session)
                        if not is_admin():
                            if not deduct_ficore_credits(db, str(current_user.id), current_user.email, 0.001, 'create_shopping_list', list_id, mongo_session):
                                delete_shopping_list(db, list_id, str(current_user.id), current_user.email, session=mongo_session)
                ...
                logger.error(f"Failed to create list: {str(e)}", extra={'user_id': str(current_user.id), 'email': current_user.email})
            ...
            try:
                ...
                if not is_admin():
                    if not check_ficore_credit_balance(db, required_amount=required_credits, user_id=str(current_user.id), email=current_user.email):
                ...
                with db.client.start_session() as mongo_session:
                    with mongo_session.start_transaction():
                        ...
                        for item in items:
                            ...
                            create_shopping_item(db, item_data, session=mongo_session)
                            total_spent += item_data['price'] * item_data['quantity']
                            if not is_admin():
                                if not deduct_ficore_credits(db, str(current_user.id), current_user.email, 0.1, 'add_shopping_item', item_id, mongo_session):
                                    flash(trans('shopping_credit_deduction_failed', default='Failed to deduct credits for item.'), 'danger')
                                    return redirect(url_for('shopping.main', tab='dashboard'))
                        update_shopping_list(db, list_id, str(current_user.id), current_user.email, {
                            'status': 'saved',
                ...
                logger.error(f"Error saving list {list_id}: {str(e)}", extra={'user_id': str(current_user.id), 'email': current_user.email})
            ...
            try:
                ...
                logger.error(f"Error sharing list {list_id}: {str(e)}", extra={'user_id': str(current_user.id), 'email': current_user.email})
            ...
            if not shopping_list:
                ...
                return redirect(url_for('shopping.main', tab='dashboard'))
            if not is_admin():
                if not check_ficore_credit_balance(db, required_amount=0.5, user_id=str(current_user.id), email=current_user.email):
            ...
            try:
                with db.client.start_session() as mongo_session:
                    with mongo_session.start_transaction():
                        delete_shopping_list(db, list_id, str(current_user.id), current_user.email, session=mongo_session)
                        if not is_admin():
                            if not deduct_ficore_credits(db, str(current_user.id), current_user.email, 0.5, 'delete_shopping_list', list_id, mongo_session):
                ...
                logger.error(f"Error deleting list {list_id}: {str(e)}", extra={'user_id': str(current_user.id), 'email': current_user.email})
                flash(trans('shopping_list_error', default='Error deleting shopping list.'), 'danger')
            return redirect(url_for('shopping.main', tab='dashboard'))

    lists = get_shopping_lists(db, str(current_user.id), current_user.email, status='active')
    ...
    if selected_list_id and ObjectId.is_valid(selected_list_id):
        ...
        if selected_list:
            items = get_shopping_items(db, {'list_id': selected_list_id, 'user_id': str(current_user.id), 'email': current_user.email.lower()})
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if action == 'save_changes':
                ...
                existing_items = get_shopping_items(db, {'list_id': list_id, 'user_id': str(current_user.id), 'email': current_user.email.lower()})
                added = 0
                edited = 0
                deleted = 0
                for item in existing_items:
                    ...
                    if f'delete_{item_id}' in request.form:
                        db.shopping_items.delete_one({'_id': ObjectId(item_id), 'user_id': str(current_user.id), 'email': current_user.email.lower()})
                        deleted += 1
                    else:
                        new_item_data = {
                        ...
                        if any(new_item_data[key] != item[key] for key in new_item_data):
                            update_shopping_item(db, item_id, str(current_user.id), current_user.email, new_item_data)
                            edited += 1
                for i in range(1, 6):
                ...
                with db.client.start_session() as mongo_session:
                    with mongo_session.start_transaction():
                        items = get_shopping_items(db, {'list_id': list_id, 'user_id': str(current_user.id), 'email': current_user.email.lower()})
                        ...
                        if not is_admin():
                            if not deduct_ficore_credits(db, str(current_user.id), current_user.email, required_credits, 'save_shopping_list_changes', list_id, mongo_session):
        ...
        logger.error(f"Error managing list {list_id}: {str(e)}", extra={'user_id': str(current_user.id), 'email': current_user.email})
    ...
    try:
        ...
        if not is_admin():
            if not check_ficore_credit_balance(db, required_amount=0.1, user_id=str(current_user.id), email=current_user.email):
                ...
                return redirect(url_for('dashboard.index'))
        items = get_shopping_items(db, {'list_id': str(list_id), 'user_id': str(current_user.id), 'email': current_user.email.lower()})
        shopping_data = {
            'lists': [{
        ...
        with db.client.start_session() as mongo_session:
            with mongo_session.start_transaction():
                ...
                buffer.seek(0)
                if not is_admin():
                    if not deduct_ficore_credits(db, str(current_user.id), current_user.email, 0.1, 'export_shopping_list_pdf', list_id, mongo_session):
        ...
        logger.error(f"Error exporting PDF for list {list_id}: {str(e)}", extra={'user_id': str(current_user.id), 'email': current_user.email})
        flash(trans('shopping_export_error', default='Error exporting to PDF.'), 'danger')
        return redirect(url_for('shopping.main', tab='dashboard'))

@shopping_bp.errorhandler(CSRFError)
def handle_csrf_error(e):
    logger.error(f"CSRF error on {request.path}: {e.description}", extra={'user_id': str(current_user.id) if current_user.is_authenticated else 'anonymous', 'email': current_user.email if current_user.is_authenticated else 'no-email'})
    flash(trans('shopping_csrf_error', default='Form submission failed. Please refresh and try again.'), 'danger')
    return redirect(url_for('shopping.main', tab='create-list')), 400
