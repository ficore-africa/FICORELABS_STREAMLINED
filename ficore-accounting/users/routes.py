    ...
    if form.validate_on_submit():
        try:
            ...
            if db.users.find_one({'_id': username}):
                flash(trans('general_username_exists', default='Username already exists'), 'error')
                logger.warning(f"Signup failed: Username {username} already exists")
                return render_template('users/signup.html', form=form, title=trans('general_signup', lang=lang))

            if db.users.find_one({'email': email}):
                flash(trans('general_email_exists', default='Email already exists'), 'error')
...
def reset_password():
    ...
    try:
        ...
        flash(trans('general_invalid_or_expired_token', default='Invalid or expired token'), 'error')
    ...
    if form.validate_on_submit():
        try:
            ...
            user = db.users.find_one({'email': email})
            if not user:
                flash(trans('general_invalid_email', default='No user found with this email'), 'error')
                logger.warning(f"No user found with email: {email}")
            ...
            log_audit_action('reset_password', user['_id'], {'user_id': user['_id']})
...
def setup_wizard():
    try:
        ...
        lang = request.args.get('lang', user.get('language', 'en')) if user else 'en'
        if not user:
