            ...
            collection_schemas = {
                ...
                'shopping_lists': {
                    'validator': {
                        '$jsonSchema': {
                            'bsonType': 'object',
                            'required': ['name', 'user_id', 'email', 'session_id', 'budget', 'created_at', 'updated_at', 'total_spent', 'status'],
                            'properties': {
                                'name': {'bsonType': 'string'},
                                'user_id': {'bsonType': ['string', 'null']},
                                'session_id': {'bsonType': 'string'},
...
def create_shopping_list(db, list_data):
    ...
    Args:
        db: MongoDB database instance
        list_data: Dictionary containing shopping list information with required fields:
                   'name' (str), 'user_id' (str), 'email' (str), 'session_id' (str),
                   'budget' (float), 'created_at' (datetime), 'updated_at' (datetime),
                   'total_spent' (float), 'status' (str)
    ...
    """
    try:
        required_fields = ['name', 'user_id', 'email', 'session_id', 'budget', 'created_at', 'updated_at', 'total_spent', 'status']
        ...
        if not isinstance(list_data['budget'], (int, float)) or list_data['budget'] < 0:
            raise ValueError("Budget must be a non-negative number")
        if list_data['status'] not in ['active', 'saved']:
            raise ValueError("Status must be 'active' or 'saved'")
